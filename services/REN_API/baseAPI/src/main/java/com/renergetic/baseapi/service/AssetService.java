...
public class AssetService {
    ...
    public AssetDAOResponse connect(AssetConnectionDAORequest connection, boolean biDirection) {
        Asset asset = assetRepository.findById(connection.getAssetId())
            .orElseThrow(() -> new InvalidNonExistingIdException("The assets to connect don't exists"));
    ...
    public MeasurementDAOResponse assignMeasurement(Long assetId, Long measurementId) {
        ...
        Optional<Measurement> m = measurementRepository.findById(measurementId);
        if (m.isPresent()) {
            ...
            measurement.setAsset(assetRepository.findById(assetId)
                .orElseThrow(() -> new InvalidNonExistingIdException("The asset doesn't exists")));
    ...
    public List<AssetDAOResponse> getConnectedTo(Long id) {
        ...
        else if (asset.getConnections() != null && !asset.getConnections().isEmpty())
    ...
    public List<AssetPanelDAO> findAssetsPanelsByUserId(Long userId, List<ConnectionType> connectionTypes,
        ...
        return assetRepository.findByUserIdConnectionTypes(userId,
                                connectionTypes.stream().map(ConnectionType::toString).collect(Collectors.toList()),
                                offset, limit).stream()
    ...
    public AssetDAOResponse updateAssetCategory(AssetCategoryDAO category, Long assetId) {
        ...
        AssetCategory assetCategory = assetCategoryRepository.findById(category.getId()).orElse(null);
