...
public interface DemandScheduleRepository extends JpaRepository<DemandSchedule, Long> {
    ...
    @Query(value = "SELECT user_demand_schedule.* " +
            ...
            "INNER JOIN asset asset_user ON asset_user.id = asset_connection.asset_id AND asset_user.user_id = :userId " +
            "INNER JOIN user_demand_schedule ON user_demand_schedule.asset_id = asset_conn.id) " +
            "WHERE user_demand_schedule.demand_start > NOW() " +
            "AND user_demand_schedule.demand_start <= NOW() + interval '1 day' " +
            "LIMIT :limit OFFSET :offset ;", nativeQuery = true)
    public List<DemandSchedule> findByUserIdFuture24H(Long userId, long offset, int limit);
    ...
    @Query(value = "SELECT user_demand_schedule.* " +
            ...
            "INNER JOIN asset asset_user ON asset_user.id = asset_connection.asset_id AND asset_user.user_id = :userId " +
            "INNER JOIN user_demand_schedule ON user_demand_schedule.asset_id = asset_conn.id) " +
            "WHERE user_demand_schedule.demand_stop < NOW() " +
            "AND user_demand_schedule.demand_stop >= NOW() - interval '1 day' " +
            "LIMIT :limit OFFSET :offset ;", nativeQuery = true)
    public List<DemandSchedule> findByUserIdPast24H(Long userId, long offset, int limit);
    ...
    List<DemandSchedule> findByAssetIdAndDemandDefinitionIdAndDemandStartLessThanEqualAndDemandStopGreaterThanEqual(
            Long assetId, Long demandId, LocalDateTime dateTime, LocalDateTime dateTime2);
