...
public class AssetDetailsAggregation {
    public static String aggregate(String type, String key, List<AssetDetails> connectedDetails) {
        switch (type) {
            case "sum":
                return connectedDetails.stream().filter(x -> x.getKey().equals(key) && x.getValue() != null)
                        .map(x -> Double.parseDouble(x.getValue())).reduce(0.0, Double::sum).toString();
            case "min":
                return String.valueOf(connectedDetails.stream().filter(x -> x.getKey().equals(key) && x.getValue() != null)
