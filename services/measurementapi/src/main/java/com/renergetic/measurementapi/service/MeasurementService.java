...
import com.renergetic.measurementapi.service.utils.InfluxAggregation;
...
public class MeasurementService {
    ...
    public List<MeasurementDAOResponse> data(String bucket, List<String> measurements, List<String> influxFields,
                                             Map<String, List<String>> tags, String from, String to, String timeVar,
                                             Boolean cumulative, Boolean performDecumulation, Boolean getTags,
                                             Boolean onlyLatestPrediction) {
//        InfluxDBClient influxDBClient = InfluxDBClientFactory.create(influxurl, influxtoken.toCharArray(), influxorg);
        ArrayList<MeasurementDAOResponse> responses = null;
        System.out.println(Runtime.getRuntime().maxMemory());
        System.out.println(Runtime.getRuntime().freeMemory());
        System.out.println(Runtime.getRuntime().totalMemory());
        QueryApi queryApi = influxDB.getQueryApi();
        if (influxFields == null || influxFields.isEmpty()) {
            influxFields = new ArrayList<>();
            influxFields.add(null);
        }

        for (var field : influxFields) {
            ...
            if (field != null)
                fluxQuery.add(String.format("filter(fn: (r) => %s)", String.format("r[\"_field\"] == \"%s\"", field)));

            // IF ONLY FETCHING LATEST PREDICITON GENERATION
            if (Boolean.TRUE.equals(onlyLatestPrediction)) {
                List<String> predictionValues = listTagValues(bucket, "time_prediction", measurements, field == null ? null : List.of(field), tags, from, to);
...
//		fluxQuery.
            fluxQuery = InfluxAggregation.setAggregation(fluxQuery, from, to, field,cumulative);
            ...
            if (responses == null) {
                responses = new ArrayList<>(MeasurementMapper.fromFlux(tables, getTags));
            } else {
                responses.addAll(MeasurementMapper.fromFlux(tables, getTags));
            }
...
//	}
     ...
     */
    public Map<String, List<String>> listTags(String bucket, List<String> measurements, List<String> fields,
                                              Map<String, List<String>> tags, String from, String to) {
        QueryApi query = influxDB.getQueryApi();
    ...
    public List<String> listTagValues(String bucket, String tag, List<String> measurements, List<String> fields,
                                      Map<String, List<String>> tags, String from, String to) {
        QueryApi query = influxDB.getQueryApi();
    ...
    @Deprecated
    public List<MeasurementDAOResponse> select(MeasurementDAORequest measurement, String from, String to,
                                               String timeVar) {
        QueryApi query = influxDB.getQueryApi();
     ...
     */
    @Deprecated
    public List<MeasurementDAOResponse> operate(MeasurementDAORequest measurement, InfluxFunction function,
                                                String field, String from, String to, String group, String timeVar) {
        QueryApi query = influxDB.getQueryApi();
