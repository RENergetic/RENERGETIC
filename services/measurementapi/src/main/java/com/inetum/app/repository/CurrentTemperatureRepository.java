...
@Repository
public class CurrentTemperatureRepository {
	@Autowired
	...
	/**
	 * Insert a temperature data to myMeasurement table
	 * @param temperature temperature to set
	 */
    public void insert(CurrentTemperature temperature, Map<String, String> tags) {
    	Builder registry = Point.measurementByPOJO(temperature.getClass())
        		.addFieldsFromPOJO(temperature)
    ...
    /**
     * Get all temperature data from database
     * @return All temperature data from database as List
     */
    public List<CurrentTemperature> select(Map<String, String> tags) {
		List<CurrentTemperature> results = new ArrayList<CurrentTemperature>();
		String pTags = String.join(" AND ", tags.keySet().stream().map(key -> String.format("\"%s\"='%s'", key, tags.get(key))).collect(Collectors.toList()));
		pTags = '(' + pTags + ')';
		
		Query query = new Query("SELECT * FROM " + CurrentTemperature.measurement() +
				((tags != null && tags.size() > 0)? " WHERE " + pTags : ""), DATABASE);
		
		System.err.println("SELECT * FROM " + CurrentTemperature.measurement() +
				((tags != null && tags.size() > 0)? " WHERE " + pTags : ""));
		
        QueryResult queryResult = influxDB.query(query, TimeUnit.MILLISECONDS);
        results = (new InfluxDBResultMapper()).toPOJO(queryResult, CurrentTemperature.class);

 
        return results;
        //SELECT sum("temperature") FROM "myMeasurement" WHERE time >= now() - 30d and time <= now() GROUP BY time(1h) fill(null)
    }

    /**
     * SELECT A DATA BETWEEN TWO DATES
     * @param from Date of first temperature data to get | Format: yyyy-MM-dd hh:mm:ss
     * @param to Date of last temperature data to get | Format: yyyy-MM-dd hh:mm:ss
     * @return The temperature data between the from and to parameters
     */
    public List<CurrentTemperature> select(String from, String to, Map<String, String> tags) {
		List<CurrentTemperature> results = new ArrayList<CurrentTemperature>();
		...
		try {
	        ...
	        // GET RESULTS AND TRANSLATE THEM TO THE temperature MODEL
	        results = (new InfluxDBResultMapper()).toPOJO(queryResult, CurrentTemperature.class);
    ...
    /**
     * Group the temperature data and calculate the indicated operation
     ...
     * @return The sum of temperature data group by time
     */
    public List<CurrentTemperature> operate(InfluxFunction function, String from, String to, String group, Map<String, String> tags) {
		List<CurrentTemperature> results = new ArrayList<CurrentTemperature>();
		String pTags = String.join(" AND ", tags.keySet().stream().map(key -> String.format("\"%s\"='%s'", key, tags.get(key))).collect(Collectors.toList()));
		pTags = '(' + pTags + ')';
		...
		if (function != null)
			try {
				Query query = new Query(
						String.format("SELECT %s(temperature) as temperature FROM %s "
								+ "WHERE time >= %s%s%s%s",
								function.name(),
								CurrentTemperature.measurement(),
								from, 
