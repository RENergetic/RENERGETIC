...
@Service
public class ForecastHeatConsumedRepository {
	@Autowired
	 ...
	 */
    public void insert(ForecastHeatConsumed power, Map<String, String> tags) {
     ...
     */
    public List<ForecastHeatConsumed> select(Map<String, String> tags) {
		List<ForecastHeatConsumed> results = new ArrayList<ForecastHeatConsumed>();
        ...
        results = (new InfluxDBResultMapper()).toPOJO(queryResult, ForecastHeatConsumed.class);
     ...
     */
    public List<ForecastHeatConsumed> select(String from, String to, Map<String, String> tags) {
		List<ForecastHeatConsumed> results = new ArrayList<ForecastHeatConsumed>();
     ...
     */
    public List<ForecastHeatConsumed> operate(InfluxFunction function, String from, String to, String group, Map<String, String> tags) {
		List<ForecastHeatConsumed> results = new ArrayList<ForecastHeatConsumed>();
		String pTags = String.join(" AND ", tags.keySet().stream().map(key -> String.format("\"%s\"='%s'", key, tags.get(key))).collect(Collectors.toList()));
		pTags = '(' + pTags + ')';
